---
title: "Tendencias y contratendencias en las desigualdades en Argentina. 2003-2024."
author: 
  - name: "Eduardo Chávez Molina" 
    affiliation: "IIGG (UBA) - UNMdP " 
  - name: "José Rodríguez de la Fuente"
    affiliation: "IIGG(UBA) - CONICET" 
subtitle: "I Jornadas de Sociología - UNC"
format: 
  revealjs:
...
-   Mirada sobre la estructura de clases socio-ocupacionales.
...
#   select(cobhe_f, "2004", "2006", "2008", "2010", "2012", "2014", "2018", "2020", "2022")
# 
# tabla %>%
#   pivot_longer(!cobhe_f, names_to = "Año", values_to = "Porcentaje") %>%
#   mutate(Porcentaje = as.numeric(Porcentaje)) %>%
#   group_by(cobhe_f) %>%
#   summarise(Tendencia = list(Porcentaje)) %>%
#   left_join(tabla, by = "cobhe_f") %>% 
#   select(-Tendencia, everything(), Tendencia) %>%
#   gt(rowname_col = "cobhe_f") %>% 
#   tab_stubhead(label = "Clase") %>% 
#   gt_plt_sparkline(Tendencia, same_limit = F) %>% 
#   gt_theme_guardian()
...
graf <- eph %>% 
  filter(CH06 >= 18 & !is.na(cobhe) & ANO4 %in% c("2004", "2014", "2023")) %>% 
  group_by(ANO4, cobhe_f) %>% 
...
ggplot(graf, aes(x=fct_rev(cobhe_f), y=edad, group=1)) +
  ...
  labs(title = "Edad promedio y desviación típica según clase (individuos)",
       subtitle = "Argentina urbana. 2004, 2014, 2023",
...
base_long_2023 <- eph %>%
  filter(ANO4 == 2023 & CH06 >= 18) %>%
  group_by(REGION, cobhe_f) %>%
  tally(wt=PONDERA) %>%
  group_by(REGION) %>%
  mutate(percent = n/sum(n))

mapa_2023 <- merge(argentina, base_long_2023, by="REGION", all.x = TRUE)

ggplot() +
  geom_sf(data = mapa_2023, aes(group = REGION, fill = percent), color = NA) +
  geom_sf(data = provincias, fill = NA, color = NA) +
  theme_map() +
  labs(title = "Distribución de las clases sociales por región",
       subtitle = "Argentina urbana 2023",
       caption = "Fuente: Elaboración propia en base a EPH-INDEC",
       fill = "% Clase social") +
...
ggplot(graf, aes(x=ANO4, y=miembros)) +
  ...
  scale_y_continuous(limits = c(2.7,4.3), breaks = seq(2.7, 4.3, .1)) +
...
ggplot(graf, aes(x=ANO4, y=incalserv)) +
  ...
  geom_point(aes(group=cobhe_f, color=cobhe_f), size = 1.8, alpha = .6) +
  geom_line(data = prom, linetype = "dotted", size = 1.2, color = "black") +
  labs(title = "Evolución de los hogares en viviendas con calidad de los servicios sanitarios satisfactoria \nsegún clase social",
       subtitle = "Argentina urbana. 2004-2024",
       caption = "Fuente: Elaboración propia en base a EPH-INDEC",
       color = "Clase social") +
  theme(axis.title.x = element_blank(),
...
theil_evol <- eph %>%
  filter(dominancia == 1 & !is.na(cobhe) & CH06 >= 18) %>% 
  group_by(ANO4) %>% 
  summarise(theil_t = dineq::theil.wtd(itf_bis, weights = PONDIH_emp),
...
descomp_theil_lab <- descomp_theil_lab %>% 
  add_column("Año" = unique(sort(eph$ANO4)), .before = "intra_lab")
...
ggplot(graf, aes(x=Año, y=theil, colour = tipo)) +
  geom_line(linetype = 1, size = .8) +
  geom_point(size = 1.8) +
  labs(title = "Coeficiente de Theil para el ingreso per cápita familiar laboral y total (Hogares)",
       subtitle = "Argentina urbana. 2004-2024",
       caption = "Fuente: Elaboración propia en base a EPH-INDEC",
       color = "Theil") +
  theme(axis.title.x = element_blank(),
  ...
  scale_y_continuous(limits = c(.2,.5), breaks = seq(.2, .5, .05)) +
  scale_x_continuous(limits = c(2004, 2024), breaks = seq(2004, 2024, 1))
...
ggplot(graf, aes(x=Año, y=porcentaje, colour = tipo)) +
  ...
  labs(title = "Descomposición del coeficiente de Theil. Componente inter-clases (Hogares)",
       subtitle = "Argentina urbana. 2003-2022 (en porcentajes)",
       caption = "Fuente: Elaboración propia en base a EPH-INDEC",
       color = "Tipo") +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        ...
        legend.text = element_text(size = 10)) +
  scale_color_d3(labels = c("Laboral", "Total")) +
  scale_y_continuous(limits = c(15,30), breaks = seq(15, 30, 1)) +
  scale_x_continuous(limits = c(2004, 2024), breaks = seq(2004, 2024, 1))
```
...
-   Cambio significativo en la dinámica que tomó la desigualdad de ingresos entre 2003-2015 y 2016-2024.

-   Entre 2003-2015, los ingresos reales aumentaron para todas las clases socio-ocupacionales, con especial énfasis en las manuales y cuenta propia no calificada.

-   :arrow_up: y :arrow_down: de la desigualdad entre períodos.
:::
:::
